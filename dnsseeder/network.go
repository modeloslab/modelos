...
func initNetwork(jnw JNetwork) (*dnsseeder, error) {
	...
	seeder.id = wire.PearlNet(t1)

	switch seeder.id {
	case wire.MainNet:
		seeder.chainParams = &chaincfg.MainNetParams
	case wire.TestNet:
		seeder.chainParams = &chaincfg.TestNetParams
	case wire.TestNet2:
		seeder.chainParams = &chaincfg.TestNet2Params
	case wire.RegTest:
		seeder.chainParams = &chaincfg.RegressionNetParams
	case wire.SimNet:
		seeder.chainParams = &chaincfg.SimNetParams
	default:
		return nil, fmt.Errorf("unknown network magic: 0x%x", t1)
	}
