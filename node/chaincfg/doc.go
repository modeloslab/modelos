...
//
// In addition to the main Pearl network, which is intended for the transfer
// of monetary value, there also exists two currently active standard networks:
// regression test and testnet.  These networks are incompatible with each other
// (each sharing a different genesis block) and software should handle errors
...
// wire.PearlNet using ParamsForNet, but be aware that this usage is
...
//	}
//
// If an application does not use one of the three standard Pearl networks,
...
// should be unique to the network.
package chaincfg
