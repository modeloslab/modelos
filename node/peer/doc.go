...
/*
Package peer provides a common base for creating and managing network
peers.

# Overview

This package builds upon the wire package, which provides the fundamental
primitives necessary to speak the wire protocol, in order to simplify
...
A quick overview of the major features peer provides are as follows:
  ...
  - Provides a basic concurrent safe peer for handling peer-to-peer
    communications via the wire protocol
  - Full duplex reading and writing of wire protocol messages
  ...
  - Flexible peer configuration
    ...
    6. Ability to register callbacks for handling wire protocol messages
...
caller to specify things such as the user agent name and version, the network
to use, which services it supports, and callbacks to invoke when wire
...
In order to do anything useful with a peer, it is necessary to react to wire
...
For convenience, a callback hook for all of the currently supported wire
...
(https://pkg.go.dev/github.com/modelos/modelos/node/wire#hdr-Bitcoin_Improvement_Proposals)
*/
package peer
