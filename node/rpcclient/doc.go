...
pearld or Oyster by default.  However, configuration options are provided to
fall back to HTTP POST and disable TLS to support talking with RPC servers that
only provide HTTP POST access.
...
In contrast, the websocket-based JSON-RPC interface provided by pearld and
Oyster only uses a single connection that remains open and allows
...
All notifications provided by pearld require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by pearld (and
Oyster passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that pearld intentionally separates the wallet
functionality into a separate process named Oyster.  This means if you are
connected to the pearld RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, Oyster provides pass through treatment
...
The following full-blown client examples are in the examples directory:
  ...
  - bitcoincorehttp
    Connects to an RPC server using HTTP POST mode with TLS disabled and gets
  ...
  - pearldwebsockets
    ...
    block count
  - oysterwebsockets
    Connects to an Oyster RPC server using TLS-secured websockets, registers
