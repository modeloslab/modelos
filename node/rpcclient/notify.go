...
type NotificationHandlers struct {
	...
	// result of a rescan request, due to how pearld may send various rescan
	...
	// OnPearldConnected is invoked when a wallet connects or disconnects from
	// pearld.
	//
	// This will only be available when client is connected to a wallet
	// server such as Oyster.
	OnPearldConnected func(connected bool)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	// OnPearldConnected
	case btcjson.PearldConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
		if c.ntfnHandlers.OnPearldConnected == nil {
			return
		}

		connected, err := parsePearldConnectedNtfnParams(ntfn.Params)
		if err != nil {
			log.Warnf("Received invalid pearld connected "+
				"notification: %v", err)
			return
		}

		c.ntfnHandlers.OnPearldConnected(connected)
...
//
// NOTE: This is a pearld extension ported from github.com/decred/dcrrpcclient
...
// parsePearldConnectedNtfnParams parses out the connection status of pearld
// and Oyster from the parameters of a pearldconnected notification.
func parsePearldConnectedNtfnParams(params []json.RawMessage) (bool, error) {
	if len(params) != 1 {
...
//
// NOTE: This is a pearld extension and requires a websocket connection.
