...
func makeHeader(prlnet PearlNet, command string,
	payloadLen uint32, checksum uint32) []byte {

	// The length of a message header is 24 bytes.
	// 4 byte magic number of the network + 12 byte command + 4 byte
	// payload length + 4 byte checksum.
	buf := make([]byte, 24)
	binary.LittleEndian.PutUint32(buf, uint32(prlnet))
...
func TestMessage(t *testing.T) {
	...
	bh := NewBlockHeader(1, &chainhash.Hash{}, &chainhash.Hash{}, 0)
	...
	tests := []struct {
		...
		{msgGetHeaders, msgGetHeaders, pver, MainNet, 62},
		...
		{msgMerkleBlock, msgMerkleBlock, pver, MainNet, 138}, // 24 msg header + 108 header + 4 txcount + 1+1 varints
	...
	for i, test := range tests {
		...
		nw, err := WriteMessageN(&buf, test.in, test.pver, test.prlnet)
		if err != nil {
		...
		nr, msg, _, err := ReadMessageN(rbuf, test.pver, test.prlnet)
		if err != nil {
	...
	for i, test := range tests {
		// Encode to wire format.
		var buf bytes.Buffer
		err := WriteMessage(&buf, test.in, test.pver, test.prlnet)
		if err != nil {
			t.Errorf("WriteMessage #%d error %v", i, err)
			continue
		}
		...
		// Decode from wire format.
		rbuf := bytes.NewReader(buf.Bytes())
		msg, _, err := ReadMessage(rbuf, test.pver, test.prlnet)
		if err != nil {
			t.Errorf("ReadMessage #%d error %v, msg %v", i, err,
				spew.Sdump(msg))
			continue
...
func TestReadMessageWireErrors(t *testing.T) {
	...
	// otherwise accurate.
	badMessageBytes := makeHeader(prlnet, "addr", 1, 0xeaadc31c)
	...
	discardBytes := makeHeader(prlnet, "bogus", 15*1024, 0)

	tests := []struct {
		buf     []byte   // Wire encoding
		pver    uint32   // Protocol version for wire encoding
		prlnet  PearlNet // Pearl network for wire encoding
	...
	for i, test := range tests {
		...
		nr, _, _, err := ReadMessageN(r, test.pver, test.prlnet)
...
func TestWriteMessageWireErrors(t *testing.T) {
	...
	tests := []struct {
		...
		// Command too long.
		{badCommandMsg, pver, prlnet, 0, wireErr, 0},
		// Force error in payload encode.
		{encodeErrMsg, pver, prlnet, 0, wireErr, 0},
		// Force error due to exceeding max overall message payload size.
		{exceedOverallPayloadErrMsg, pver, prlnet, 0, wireErr, 0},
		// Force error due to exceeding max payload for message type.
		{exceedPayloadErrMsg, pver, prlnet, 0, wireErr, 0},
		// Force error in header write.
		{bogusMsg, pver, prlnet, 0, io.ErrShortWrite, 0},
		// Force error in payload write.
		{bogusMsg, pver, prlnet, 24, io.ErrShortWrite, 24},
	}
	...
	for i, test := range tests {
		...
		nw, err := WriteMessageN(w, test.msg, test.pver, test.prlnet)
