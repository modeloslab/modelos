...
func (msg *MsgVersion) PrlDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	buf, ok := r.(*bytes.Buffer)
	if !ok {
		return fmt.Errorf("MsgVersion.PrlDecode reader is not a " +
			"*bytes.Buffer")
	}
...
// This is part of the Message interface implementation.
func (msg *MsgVersion) PrlEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	...
	// The wire encoding for the relay field is true when transactions should
	// be relayed, so reverse it from the DisableRelayTx field.
...
// NewMsgVersion returns a new version message that conforms to the
