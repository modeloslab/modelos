...
impl PublicProofParams {
    ...
    pub fn commitment_hash(&self, job_key: Hash256) -> (Hash256, Hash256) {
        // V3 (Salted) binds each Merkle root to its own domain salt AND its dimension before the
        // seed chain; Legacy passes the roots through unchanged. MoE salts BEFORE the routing fold,
        // so the bound root (not the raw one) enters hash_activations.
...
impl PublicProofParams {
    ...
    /// `seed_derivation` is NOT carried on the wire (the wire always holds raw roots) — it comes
    /// from the block's certificate version, so the caller must supply it.
