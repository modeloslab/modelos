...
//!
//! Ported from upstream Pearl fc5ca65a1 (salted noise-seed hardfork). The salts and
//! the bind-message layout are consensus-critical and must stay byte-identical to
//! upstream, or PRL blocks we mine are rejected (and vice versa).
...
mod tests {
    ...
    /// Legacy must be the identity on the roots — any change here silently forks
    /// every pre-V3 block we still verify.
    #[test]
    fn legacy_is_identity() {
        let a = [0xAAu8; 32];
        let b = [0xBBu8; 32];
        assert_eq!(SeedDerivation::Legacy.bind_roots(&a, &b, 192, 320), (a, b));
    }

    /// The dimension is committed: the same root under a different m/n must bind differently.
    #[test]
    fn binding_commits_the_dimension() {
        let a = [0xAAu8; 32];
        assert_ne!(bind_root_a(&a, 192), bind_root_a(&a, 193));
        let b = [0xBBu8; 32];
        assert_ne!(bind_root_b(&b, 320), bind_root_b(&b, 321));
    }

    /// A and B use different salts, so an A-root and a B-root with the same bytes
    /// and dimension must not collide (this is what upstream's fork closes).
    #[test]
    fn sides_are_domain_separated() {
        let root = [0xCCu8; 32];
        assert_ne!(bind_root_a(&root, 256), bind_root_b(&root, 256));
    }
}
...
mod upstream_parity_tests {
    ...
    /// Pinned vectors copied from upstream Pearl fc5ca65a1 (`zk-pow/src/api/seed.rs`).
    /// These are CONSENSUS: if they drift, blocks we mine are rejected by Pearl and vice versa.
    #[test]
