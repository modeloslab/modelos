...
fn test_ffi_mine_prove_verify_with_rank(rank: u16, is_moe: bool) {
    ...
    let start = Instant::now();
    let pow_proof: PlainProof = mine_moe(m, n, k, block_header, mining_config, None, false, SeedDerivation::Legacy).expect("Mining failed");
    ...
    let start = Instant::now();
    let (private_params, mut public_params) = pow_proof.parse_proof(block_header, SeedDerivation::Legacy).expect("Failed to parse plain proof");
