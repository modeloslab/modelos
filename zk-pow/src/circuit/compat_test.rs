...
mod test {
    ...
    fn starky_hash(block_header: IncompleteBlockHeader, plain_proof: &PlainProof) -> String {
        ...
        let (private_params, public_params) = plain_proof.parse_proof(block_header, SeedDerivation::Legacy).expect("Failed to parse plain proof");
    ...
    fn generate_plain_proof(p: &Params) -> PlainProof {
        ...
        try_mine_one(&mut rng, p.m, p.n, p.k, p.block_header, p.mining_config, None, false, SeedDerivation::Legacy)
            .unwrap()
            .unwrap()
    }
    ...
    fn generate_and_write_stark_proof(proof_path: &Path) {
        ...
        let p = params();
        let plain_proof = generate_plain_proof(&p);
        let result = prove::zk_prove_plain_proof(p.block_header, &plain_proof, &mut cache, false, SeedDerivation::Legacy).expect("Proving failed");
    ...
    fn test_proof_fixture() {
        ...
        let (public_params, proof) = ZKProof::deserialize(params.block_header, SeedDerivation::Legacy, public_data, proof_data).unwrap();
    ...
    fn generate_and_write_moe_proof(proof_path: &Path) {
        ...
        let mut cache = <PearlRecursion as RecursionCircuit>::CircuitCache::default();
        let result = prove::zk_prove_plain_proof(header, &plain_proof, &mut cache, false, SeedDerivation::Legacy).expect("MoE proving failed");
