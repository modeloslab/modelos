...
pub fn try_mine_one_moe<R: Rng>(
    ...
    if config.moe.is_none() {
        return try_mine_one(rng, m, n, k, header, config, signal_range, wrong_jackpot_hash, seed_derivation);
    }
    ...
    let (b_noise_seed, a_noise_seed, routing_end_offsets) =
        compute_commitment_hash_with_offsets(
            &job_key,
            &a_row_major,
            &b_col_major,
            Some(&routing[..]),
...
pub fn mine(
    ...
    loop {
        let proof = try_mine_one(&mut rng, m, n, k, header, config, signal_range, wrong_jackpot_hash, seed_derivation)?;
...
pub fn mine_moe(
    ...
    loop {
        let proof = try_mine_one_moe(&mut rng, m, n, k, header, config, signal_range, wrong_jackpot_hash, seed_derivation)?;
        if let Some(proof) = proof {
            return Ok(proof);
        }
    }
